...
export class CatalogSearchService {
  ...
  async reindexPublished(): Promise<void> {
    ...
    for (const movie of movies) await this.indexCurrentMovie(movie.id);
  }

  private async request(method: string, path: string, body?: unknown): Promise<Response> {
    try {
      return await fetch(`${this.config.opensearchUrl}${path}`, { method, headers: body === undefined ? undefined : { 'content-type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body), signal: AbortSignal.timeout(4_000) });
