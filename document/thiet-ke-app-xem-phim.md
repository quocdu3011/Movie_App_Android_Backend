...
Bản đồng bộ: **2026-09-12 / revision 2**. Tài liệu mô tả sản phẩm và kiến trúc, chưa triển khai code. Phạm vi không bao gồm kiểm tra bản quyền nguồn bên thứ ba.

[Thiết kế backend chi tiết](backend-chi-tiet.md) là nguồn chuẩn của schema, API nội bộ và sự kiện. [Tài liệu tích hợp KKPhim](kkphim-api.md) mô tả endpoint/response từ provider. [TODO và prompt backend](todo-prompt-backend.md) là lộ trình thực hiện. Khi thay đổi hợp đồng dự án, cập nhật phần sản phẩm và TODO liên quan.
...
| Backend | Lựa chọn |
|---|---|
| Ngôn ngữ/framework | Node.js 24.21.0 LTS + NestJS 11.2.3/TypeScript 5.9.3, TypeORM 0.3.31 migrations |
| API | REST/JSON công khai và HTTP REST nội bộ có service auth |
| Dữ liệu | PostgreSQL 16.4 ở Auth dev, database riêng từng service; Redis cache/lease, OpenSearch |
| Sự kiện | Kafka KRaft với outbox/inbox, idempotency |
| Video | S3-compatible/MinIO, FFmpeg, HLS, media-edge/CDN adapter |
| Dev/CI | Docker Compose, GitHub Actions; Kubernetes sau MVP |
| Giám sát | Structured logs, Prometheus/Grafana/Loki |

Dùng NestJS monorepo tại `Backend/`, gồm Gateway, Auth, Profile, Catalog, Payment, Streaming, Worker, Notification và Recommendation. Chia domain từ đầu; chỉ chạy các service cần thiết ở từng giai đoạn. Chi tiết workspace, pin version và deployment nằm trong tài liệu backend.
