...
Bản đồng bộ: **2026-09-12 / revision 2**. Đây là đặc tả thiết kế, chưa phải mã nguồn hay migration đã chạy. Phạm vi không bao gồm kiểm tra bản quyền nguồn bên thứ ba.
...
|---|---|
| Backend | Node.js 24.21.0 LTS, NestJS 11.1.8 + TypeScript 5.9.3 |
| ORM | TypeORM 0.3.27 + migration theo database; `synchronize=false` ở mọi môi trường dùng chung |
| Đồng bộ | HTTP REST/JSON qua Gateway; HTTP REST nội bộ có xác thực service |
| Bất đồng bộ | Kafka KRaft, giao nhận ít nhất một lần; outbox/inbox và consumer idempotent |
| Dữ liệu | PostgreSQL 16.4 cho Compose Auth dev; kiểm tra nâng phiên bản trước môi trường dùng chung |
| Cache/lease | Redis; MVP không dùng Redis làm nơi duy nhất lưu tiến độ đã xác nhận thành công |
| Search | OpenSearch, index từ catalog nội bộ đã chuẩn hóa |
| Storage/media | S3-compatible; MinIO dev, FFmpeg worker, media-edge dev; adapter CDN cho triển khai thật |
| Vận hành | Docker Compose; GitHub Actions; structured logs, Prometheus/Grafana/Loki |

Pin phiên bản công cụ/image và lockfile ở giai đoạn 0, không dùng tag `latest`. Chọn phiên bản tương thích thực tế thay vì xem những tên version trong tài liệu là đã được kiểm thử.

Workspace bắt đầu G1 đã pin các package và có `package-lock.json`, nhưng G0 tổng thể chưa hoàn tất (các service nền khác, Kafka/Redis/CI còn thiếu). `.nvmrc` chọn Node 24.21.0 LTS; máy thực hiện hiện chạy Node 26.7.0 nên build ở máy đó chỉ là kiểm tra tương thích bổ sung, CI sau này phải chạy Node 24 LTS.
...
└── Backend/
    ...
    │   └── content-provider/             # KKPhim HTTP client + mapper, không chứa entity DB
    ├── docker/                          # DB init, media-edge, Dockerfiles
    ├── docker-compose.yml
    └── package.json
```
...
| POST | `/internal/profiles/validate` | Profile | Streaming, Gateway, Catalog, Profile composition; userId/profileId |
...
- CI publish image chỉ trusted main/tag, PR chạy checks không có registry/deploy secrets. Shared lib/root config/lockfile đổi phải rebuild tất cả app phụ thuộc (chưa có dependency graph thì build tất cả), không chỉ app paths. Workflow nằm `.github/workflows` ở repository root, workdir Backend.
...
Các hợp đồng đã được đối chiếu ở mức tài liệu. Chưa có backend để build/test tải hoặc chứng minh provider/CDN tương thích thật; các tiêu chí đó nằm trong TODO, không được đánh dấu hoàn thành chỉ vì đã viết thiết kế.

Tham khảo kỹ thuật: [NestJS workspace](https://docs.nestjs.com/cli/monorepo), [PostgreSQL constraints](https://www.postgresql.org/docs/current/ddl-constraints.html), [TypeORM migrations](https://typeorm.io/docs/advanced-topics/migrations/), [OAuth refresh-token security](https://www.rfc-editor.org/rfc/rfc9700.html), [CloudFront signed cookies](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-choosing-signed-urls-cookies.html). Các quyết định domain/TTL/thứ tự giai đoạn là thiết kế của dự án, không phải yêu cầu từ những tài liệu ngoài này.
