...
- `favorites(profile_id UUID FK profiles, movie_id UUID, created_at)` với PK `(profile_id,movie_id)`. PUT thao tác thêm là idempotent.
...
- Catalog có public list/search/detail/home qua Gateway và Admin APIs theo bảng mục 4.3. User/profileId đi qua Gateway JWT/session, Profile ownership validation và kids filtering; personalized responses là `no-store`, public response không mang profile state. Search G3 ban đầu dùng PostgreSQL `ILIKE`; G7 thay bằng OpenSearch.
...
### 8.8. G7 Search, favorites/history và Home composition — implementation và bằng chứng local

- Catalog tạo index OpenSearch `movieapp_catalog_v1` với analyzer `vi_text` (standard tokenizer, lowercase và asciifolding), mapping keyword cho filter/sort và tie-break `id`. Search chỉ đọc index Catalog đã import; sau mỗi query hydrate từ Catalog DB hiện tại để lọc archive/kids stale, bù page từ tập hit còn hợp lệ và trả total cùng tập đó.
- Consumer `catalog-search-v1` đọc `movie.published`, `movie.updated`, `movie.archived` và `movie.source.updated`; inbox dedupe theo eventId, projection version lưu theo movie. Reindex bootstrap từ Catalog DB hiện tại; khi event đến lệch thứ tự, projection lấy trạng thái hiện tại, nên event publish cũ không làm revived movie archive xuất hiện lại. Index không có URL phát.
- Profile migration thêm `favorite_movies(profile_id,movie_id,created_at)` với PK để PUT idempotent. Profile xác minh ownership/kids trước khi gọi batch Catalog nội bộ; favorites hydrate Catalog, còn watch-history đọc Streaming rồi hydrate Catalog. Movie không còn public hoặc không phù hợp kids được trả `movie=null,tombstone=true`, không join DB giữa service.
- Gateway thêm `GET /home?profileId=...`, qua Profile xác minh owner rồi compose `continue_watching`, catalog new releases và recommendation. Response luôn `Cache-Control: no-store`; Recommendation lỗi/chưa có trả section `fallback_new_releases`. `/catalog/home` bỏ profileId trước khi forward, vẫn public và không có state cá nhân.
- Bằng chứng local 2026-09-13: `npm run smoke:g7-ci` exit code 0 trên PostgreSQL/Redis/Kafka/OpenSearch Compose và chạy lại exit code 0 trong `node:24.21.0-bookworm`. E2E khởi động Auth, Profile, Catalog, Streaming, Gateway thật và xác nhận query tiếng Việt có/không dấu, pagination ổn định, recheck kids, archive/reindex và event cũ không revive; favorite unique/owner isolation; history tombstone/hydration; Home no-store fallback khi Recommendation không chạy; public Catalog home không lộ profile/progress/raw playback URL. GitHub Actions hosted chưa được trigger.

### 8.9. G8 Notification, Recommendation và telemetry — implementation và bằng chứng local

- Notification migration tạo inbox event và delivery bền vững; khóa unique `(event_id, recipient_id, channel)` giữ đúng một bản ghi cho mỗi đích. Consumer chỉ nhận `movie.published`, `payment.success`, `subscription.expiring` và `video.transcode_failed`; policy MVP lần lượt gửi push cho catalog announcements, email cho user thanh toán, push+email khi subscription sắp hết hạn và email cho content admins khi transcode lỗi. Mock log chỉ ghi delivery ID, channel và event type, không ghi recipient hoặc payload.
- Delivery worker claim row pending có khóa, xử lý mock channel, retry theo backoff bounded rồi chuyển `dlq`. Endpoint nội bộ chỉ Gateway được gọi để replay row DLQ; replay đặt lại cùng delivery về pending, không insert delivery mới. Vì đây là mock, việc giao provider thật và trạng thái delivery-unknown vẫn là phần tích hợp sau MVP.
- Recommendation migration lưu inbox theo consumer/event và `watch_events` unique cả event lẫn session. Consumer chỉ đếm `playback.qualified`; progress không đi vào service này. Trending dùng distinct session trong bảy ngày; gợi ý đọc profile ownership/kids và Catalog nội bộ, lấy phim published cùng genre chưa xem rồi fallback Trending/new releases. Catalog hydrate/recheck publication và Kids trước khi trả dữ liệu; Gateway đặt section này vào Home, nhưng Home vẫn fallback nếu dependency lỗi.
- Cả hai consumer có health/readiness, request ID và `/metrics` Prometheus text nội bộ. Notification đo số delivery theo `status`; Recommendation đo tổng qualified view từ DB. Không có metric label theo `userId` hoặc `movieId`; DLQ/retry quan sát qua trạng thái delivery, metric và log mock an toàn. Event `profile.deleted` xóa watch events liên quan.
- Bằng chứng local 2026-09-13: `npm run smoke:g8-ci` pass trên PostgreSQL/Kafka Compose, sau đó chạy lại trong `node:24.21.0-bookworm` qua host network với exit code 0. E2E chạy Auth, Profile, Catalog, Streaming, Notification, Recommendation và Gateway thật; kiểm tra replay event/session, Trending 7 ngày, genre/Kids filtering, notification unique/retry/DLQ/replay, Recommendation section trong Home và cleanup profile. Test restart cả hai consumer, phát lại event cũ để xác nhận không duplicate, rồi phát event mới để xác nhận consumer hoạt động trở lại. GitHub Actions hosted chưa được trigger.

### 8.10. G9 Nghiệm thu hệ thống trước deploy — implementation và bằng chứng local

- `npm run test:e2e` chạy contract check và bảy E2E có sẵn theo runner timeout 180 giây/scenario: Profile quota/ownership, KKPhim import/sync, Payment HMAC/concurrency/expiry, Streaming third-party, owned upload/transcode/media edge, Search/Home/history và Notification/Recommendation. Mỗi scenario tự khởi động service thật với fixture offline, poll bounded và cleanup; runner báo thời gian và fail-fast nếu một scenario lỗi.
- G9 xác nhận HLS third-party bằng HTTPS fixture manifest/segment và HLS owned bằng MinIO/Worker/Media edge master/variant/segment thật cùng ffprobe; không coi `VIDEO_NOT_READY` hay playback URL string là thành công. Authorization/role/internal token, private/host URL policy, raw webhook HMAC, quota races, out-of-order/archive, provider outage/recovery, worker restart/generation và replay inbox/session/delivery đều có assertion từ các scenario.
- Load local ngày 2026-09-13 trên Linux x64, Node 26.7.0, 12 CPU logic/15,330 MiB: 50 progress writes concurrency 10 p95 111.03 ms; 50 concurrent request `GET /catalog/home` p95 200.02 ms và route trả `public, max-age=300`. Đây là warm origin measurement của route cacheable, không phải CDN cache hit hay load benchmark production. Resource snapshot sau suite: PostgreSQL 74.04 MiB, Redis 15.59 MiB, Kafka 572.8 MiB, OpenSearch 1.324 GiB, MinIO 278.1 MiB.
- Report cuối: 8 scenario trong 274,143 ms, outbox pending Profile/Catalog/Payment/Streaming `0,0,0,0`, Kafka lag Catalog Search/Recommendation/Notification `0,0,0`. Consumer dùng `fromBeginning:false`; offset `-1` ở partition chưa có record mới được ghi là latest policy, không tính backlog lịch sử. Contract Gateway/OpenAPI/TODO và khác biệt fixture KKPhim ghi tại [g9-api-contract.md](g9-api-contract.md) và [kkphim-api.md](kkphim-api.md). Không deploy ở G9; không có Android app/Media3 device test, CDN hay provider production evidence.

### 8.11. G10 Đóng gói và chuẩn bị staging

- `docker/backend/Dockerfile` dùng các stage dependencies, build, production dependencies và runtime. Mỗi service được gắn `MOVIEAPP_SERVICE` vào cùng entrypoint allow-list, chạy `dist/apps/<service>/main.js` dưới user `node`; target riêng `worker-runtime` thêm FFmpeg, còn target `migrations` chỉ chạy `scripts/run-migrations.mjs` tuần tự một lần cho tám database.
- `docker-compose.staging.yml` chỉ mô tả ứng dụng; PostgreSQL, Redis, Kafka, OpenSearch và MinIO được cấp như dependency staging bên ngoài qua biến môi trường. Service có read-only filesystem, tmpfs `/tmp`, `no-new-privileges`, resource defaults và healthcheck. Migration là profile riêng nên operator chạy nó hoàn tất trước rollout, không có replica nào tự chạy migration khi khởi động.
- Private key JWT chỉ mount qua Compose secret `auth_private_key` cho `auth-service`; Streaming và Worker không nhận secret đó. Payment staging đặt `PAYMENT_MOCK_ENABLED=false`. `.env.staging.example` chỉ có placeholder và đường dẫn secret, không có credential thật.
- CI tính phạm vi thay đổi. Khi app, shared library, lockfile hoặc cấu hình root đổi, CI chạy toàn bộ workspace vì chưa có dependency graph chính thức; PR chỉ kiểm tra/build, còn publish `ghcr.io` dùng tag `sha-${github.sha}` chỉ trên push `main` hoặc tag trusted. Tài liệu workflow không đồng nghĩa đã publish image.
- Quy trình rollback là rollout lại image SHA trước đó sau khi kiểm tương thích schema; migration hiện chỉ append và `synchronize=false`. `scripts/staging-backup-restore-rehearsal.mjs` dùng custom `pg_dump`/`pg_restore` vào database tạm và put/get/delete/put lại một object MinIO probe để kiểm tra restore byte-for-byte. Không coi rehearsal local là backup retention hoặc DR plan production.
- Bằng chứng local 2026-09-13: 11 image `sha-94ee8f606ffb` build thành công. Compose staging render được bằng `.env.staging.example`; artifact assertions, lint, typecheck và build pass. Smoke image xác nhận 10 runtime image chạy user `node`, Worker có FFmpeg và Auth/media-edge container trả health với dependency Compose local. Migration image áp dụng đủ tám command vào tám database trống tạm rồi xóa các database đó. Restore rehearsal pass với custom Auth dump/restore và MinIO object restore byte-for-byte. Không có deploy staging trong phiên này vì chưa được cung cấp registry/prefix, hostname/TLS/ingress, CDN/object storage staging hoặc secret target; chưa có endpoint staging để smoke catalog/playback.
