...
Bản đồng bộ: **2026-09-12 / revision 2**. Checklist chưa tick là việc chưa có bằng chứng nghiệm thu. G1 hiện đã có một phần mã nguồn nhưng chưa qua integration/e2e với PostgreSQL; G0 tổng thể cũng chưa hoàn tất.
...
- Làm trong `Backend/`, giữ `document/`; workflow GitHub Actions ở `.github/workflows/` của repository root. Không tạo thêm workspace lồng `movie-backend`.
...
- [ ] Khởi tạo NestJS monorepo ngay tại Backend và chốt version/lockfile.
- [ ] Tạo apps/libs đúng backend mục 1, chưa cài business logic giả như thể đã hoàn thành.
- [ ] Compose profile core/media/observability và bootstrap idempotent.
- [ ] Health/readiness, requestId, shared response/error/config và route skeleton.
- [ ] CI lint/typecheck/unit/build từ repository root, đường dẫn Backend đúng.
...
- [ ] users/auth_sessions/refresh_tokens migrations và email unique không phân biệt hoa thường.
- [ ] Register/login/refresh/logout/JWKS theo hợp đồng chuẩn.
- [ ] Password hash, rotation/reuse detection, giới hạn device atomic.
- [ ] Service auth và Gateway/service guards, revoke/user status.
- [ ] Dev seed admin bằng env, không production seed/không private key ngoài Auth.
...
**Trạng thái hiện tại:** đã có mã nguồn G1 và smoke script tại `Backend/test/auth-smoke.mjs`; chưa đánh dấu nghiệm thu. Workspace chỉ mới có nền tối thiểu cho hai app Auth/Gateway, chưa hoàn tất G0. Cần chạy migration và smoke test với PostgreSQL thật; G0 Compose/CI đầy đủ vẫn là công việc riêng. `npm audit --omit=dev` còn báo Multer high từ adapter Express; G1 không có upload/multipart route, nhưng cần nâng adapter/dependency hoặc chuyển adapter trước khi thêm nhận file hay deploy production.

## G2 — Profile core

- [ ] Migrations profiles/profile_quotas và soft delete.
- [ ] CRUD, ownership và giới hạn 5 profile atomic.
- [ ] Internal validation cho Streaming/Gateway, event profile.deleted qua outbox.
- [ ] Gateway routes và contract tests.
